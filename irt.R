...
full_df <- grades %>%
  ...
  select(class_id, student_id, question_number, correct, process) 
...
long_df <- bind_rows(full_df, pretest_filtered) %>%
  group_by(class_id) %>%
  filter(length(unique(student_id)) > 1) %>%
  ungroup() %>%
  mutate(
    question_id        = if_else(str_detect(question_number, "pretest"),
                                 question_number,
                                 paste0(class_id, question_number)),
    ...
    process            = factor(process, levels = c("gan", "stats", "pretest"))
  )
...
model {
  ...
  // Likelihood
  for (n in 1:n_obs) {
    real eta = a[item[n]] * ((theta[person[n]] + mu_class[classes_obs[n]]) 
                             - b[item[n]]);
...
# Theta (all persons, typically not split by process)
theta_df     <- tibble(theta = theta_hat)

theta_lookup <- long_df %>%
  filter(process != "pretest") %>%
  select(student_id, student_id_factor, process) %>%
  distinct() 

theta_df <- theta_lookup %>%
  arrange(student_id_factor) %>%  # must match ordering used in Stan
  mutate(theta = theta_hat)

ggplot(theta_df, aes(x = theta, fill = as.factor(class_id))) +
  geom_density(alpha = 0.3) +
  labs(title = "Person Ability (theta) by Class", x = "theta", fill = "Class") +
  theme_minimal()

# Discrimination - AI
ggplot(a_ai_df, aes(x = a)) +
  geom_density() +
  labs(title = "AI Discrimination (a)", x = "a") +
  theme_minimal()

# Discrimination - Stats
ggplot(a_stats_df, aes(x = a)) +
  geom_density() +
  labs(title = "Stats Discrimination (a)", x = "a") +
  theme_minimal()

# Discrimination - Pretest
ggplot(a_pretest_df, aes(x = a)) +
  geom_density() +
  labs(title = "Pretest Discrimination (a)", x = "a") +
  theme_minimal()

# Difficulty - AI
ggplot(b_ai_df, aes(x = b)) +
  geom_density() +
  labs(title = "AI Difficulty (b)", x = "b") +
  theme_minimal()

# Difficulty - Stats
ggplot(b_stats_df, aes(x = b)) +
  geom_density() +
  labs(title = "Stats Difficulty (b)", x = "b") +
  theme_minimal()

# Difficulty - Pretest
ggplot(b_pretest_df, aes(x = b)) +
  geom_density() +
  labs(title = "Pretest Difficulty (b)", x = "b") +
  theme_minimal()

# Person abilities (theta)
ggplot(theta_df, aes(x = theta)) +
  geom_density() +
  labs(title = "Person Ability (theta)", x = "theta") +
  theme_minimal()
