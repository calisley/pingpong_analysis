...
# 9. Compile and sample — tweaked controls for fewer divergences and faster tuning
fit <- stan(
  model_code = stan_code,
  data       = stan_data,
  iter       = 2000,           # half as many total iters
  warmup     = 1000,           # half as many warmup
  chains     = 3,
  control    = list(
    adapt_delta   = 0.95,      # reduce divergences
    max_treedepth = 12         # avoid depth warnings
  )
)

saveRDS(fit, here("models","paper_model_trees.rds"))
